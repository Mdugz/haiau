...
const store = {
  high:+localStorage.getItem("haiAuHigh")||0,
  coins:+localStorage.getItem("haiAuCoins")||0,
  amulets:+localStorage.getItem("haiAuAmulets")||0,
  save(){localStorage.setItem("haiAuHigh",this.high);localStorage.setItem("haiAuCoins",this.coins);localStorage.setItem("haiAuAmulets",this.amulets)}
};
...
const input={x:0,y:0,keys:new Set(),active:false};
...
function joyMove(e){const r=joystick.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,max=r.width*.34;let dx=e.clientX-cx,dy=e.clientY-cy,d=Math.hypot(dx,dy);if(d>max){dx*=max/d;dy*=max/d}input.x=dx/max;input.y=dy/max;stick.style.transform=`translate(${dx}px,${dy}px)`}
joystick.addEventListener("pointerdown",e=>{joyId=e.pointerId;joystick.setPointerCapture(joyId);joyMove(e)});
joystick.addEventListener("pointermove",e=>{if(e.pointerId===joyId)joyMove(e)});
joystick.addEventListener("pointerup",()=>{joyId=null;input.x=input.y=0;stick.style.transform="translate(0,0)"});
joystick.addEventListener("pointercancel",()=>{joyId=null;input.x=input.y=0;stick.style.transform="translate(0,0)"});
...
class Boss{
  ...
  firePattern(){
    const gap=Math.floor(rand(1,5));for(let i=0;i<7;i++)if(i!==gap)game.projectiles.push(new Projectile(W+20,80+i*(H-160)/6,-260*(this.slow?0.5:1),0,10,"fire"))
  }
  fireLaser(){audio.sfx("laser");shake=14;game.laser={lane:this.lane,t:0,dur:.65}}
  fireRadial(){
    // Đạn tỏa từ tâm Boss, xếp theo các tia từ tâm đến đường tròn.
    // Các tia có nhiều hạt nối tiếp nhau tạo thành một vòng đạn đang mở rộng.
    const count=12;
    const bulletsPerRay=4;
    const base=rand(0,TAU);
    const speed=this.slow?150:300;
    for(let i=0;i<count;i++){
      const a=base+i*TAU/count;
      for(let j=1;j<=bulletsPerRay;j++){
        const delay=(j-1)*0.045;
        const p=new Projectile(this.x+Math.cos(a)*j*8,this.y+Math.sin(a)*j*8,
          Math.cos(a)*speed,Math.sin(a)*speed,8,"fire");
        p.life=5;
        p.delay=delay;
  ...
  fireGrid(){
    const gap=Math.floor(rand(2,6));for(let i=0;i<9;i++)if(i!==gap)game.projectiles.push(new Projectile(W+10,55+i*(H-110)/8,-310*(this.slow?0.5:1),0,9,"fire"))
  }
  damage(n,type){
...
class Game{
  ...
  spawnObstacle(){
    const y=rand(70,H-70);this.projectiles.push(new Projectile(W+30,y,-rand(120,190)*(this.stage===1?1.3:1),0,rand(14,20),"fire"));
  ...
  revive(){if(store.amulets<=0)return;store.amulets--;store.save();this.player.lives=1;this.player.inv=3;this.running=true;hideAll();$("hud").classList.remove("hidden");$("joystick").classList.remove("hidden");audio.start(this.stage);showToast("BÙA HỘ MỆNH — BẤT TỬ 3 GIÂY")}
...
function hideAll(){["menu","shop","gameover","victory"].forEach(id=>$(id).classList.add("hidden"));$("hud").classList.add("hidden");$("joystick").classList.add("hidden")}
...
async function forceLandscape(){
  ...
  }catch(_){/* Một số trình duyệt chỉ cho khóa hướng sau khi fullscreen */}
}

$("playBtn").onclick=async()=>{audio.init();await forceLandscape();game.start()};
